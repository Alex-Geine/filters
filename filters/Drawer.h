...
public:
	...
	// Нарисовать график по переданным данным.
	void DrawSix(
		vector<double>& data, vector<double>& data1, vector<double>& data2, vector<double>& data3, vector<double>& data4, vector<double>& data5, 
		double data_x_min, double data_x_max, double data_y_min, double data_y_max, char A1, char A2, vector<double>& keys = vector<double>())
	{
		if (!init) return;
		...
		CPen grid_pen(PS_SOLID, 1, RGB(0, 0, 0));
		CPen data_pen(PS_SOLID, 2, RGB(255, 0, 0));
		CPen data1_pen(PS_SOLID, 2, RGB(0, 255, 0));
		CPen data2_pen(PS_SOLID, 2, RGB(0, 0, 255));
		CPen data3_pen(PS_SOLID, 2, RGB(255, 250, 0));
		CPen data4_pen(PS_SOLID, 2, RGB(255, 0, 255));
		CPen data5_pen(PS_SOLID, 2, RGB(0, 250, 255));
		CPen points_pen(PS_SOLID, 2, RGB(0, 0, 255));
		CFont font;
		font.CreateFontW(15, 0, 0, 0,
			FW_DONTCARE,
		...
		vector<double> y1 = convert_range(data1, actual_top, actual_bottom, data_y_max, data_y_min);
		vector<double> y2 = convert_range(data2, actual_top, actual_bottom, data_y_max, data_y_min);
		vector<double> y3 = convert_range(data3, actual_top, actual_bottom, data_y_max, data_y_min);
		vector<double> y4 = convert_range(data4, actual_top, actual_bottom, data_y_max, data_y_min);
		vector<double> y5 = convert_range(data5, actual_top, actual_bottom, data_y_max, data_y_min);
		vector<double> x = convert_range(keys, actual_right, actual_left, data_x_max, data_x_min);
		...
		memDC.MoveTo(x[0], y3[0]);
		for (unsigned int i = 0; i < y.size(); i++)
		{
			memDC.LineTo(x[i], y3[i]);
		}
		memDC.SelectObject(&data4_pen);
		memDC.MoveTo(x[0], y4[0]);
		for (unsigned int i = 0; i < y.size(); i++)
		{
			memDC.LineTo(x[i], y4[i]);
		}
		memDC.SelectObject(&data5_pen);
		memDC.MoveTo(x[0], y5[0]);
		for (unsigned int i = 0; i < y.size(); i++)
		{
			memDC.LineTo(x[i], y5[i]);
		}
